...
class FeedFragment : Fragment() {
    ...
    private lateinit var recyclerViewAdapter: RecipeAdaptor
    ...
    private val itemTouchHelper by lazy {
        val simpleItemTouchCallback = object : ItemTouchHelper.SimpleCallback(
            ...
            override fun onMove(
                ...
                val adapter = recyclerView.adapter as RecipeAdaptor
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.navigateToRecipeEditOrAddScreenEvent.observe(this) {
            val direction = FeedFragmentDirections.toNewRecipeFragment(
                0,
                NewRecipeFragment.REQUEST_FEED_KEY
            )
            findNavController().navigate(direction)
        }

        viewModel.navigateToCurrentRecipeScreenEvent.observe(this) { currentRecipe ->
            val direction = FeedFragmentDirections.toSingleRecipeFragment(currentRecipe.id)
            findNavController().navigate(direction)
        }
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if (item.itemId == R.id.filter) {
            val direction =
                FeedFragmentDirections.feedFragmentToFilterFragment()
            findNavController().navigate(direction)
        }
    ...
    override fun onCreateView(
        ...
        recyclerViewAdapter = RecipeAdaptor(viewModel)
